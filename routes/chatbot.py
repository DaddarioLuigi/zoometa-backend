...
@chatbot_bp.route("/chat_with_audio", methods=["POST"])
def chat_with_audio():
    """Handle a text message and return both text and audio response."""
    data = request.json
    session_id = data.get("session_id") if data else None
    user_input = data.get("input") if data else None
    response_format = data.get("response_format", "html") if data else "html"
    ...
    response_text, audio_base64 = chat_service.handle_user_query_with_audio(
        user_input, response_format
    )

    conversation = Conversation(
        session_id=session_id, user_input=user_input, bot_response=response_text
    )
    db.session.add(conversation)
    db.session.commit()
